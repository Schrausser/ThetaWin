...
double seed, seed1;
double zf_wert[2000], s_wert[2000], s11_wert[2000], s12_wert[2000], s21_wert[2000], s22_wert[2000], sr_wert[2000], Q_wert[2000];

int a_THETS11, a_THETS21, a_TTHR1;
int a_THETS12, a_THETS22, a_TTHR2;
int a_TTHA, a_THETP, a_THETQ, sw4, iLauf; 
...
double   a_MAX, a_MIN, _S[9], d_[9], d_sum[9], sum[9], sum2[9], qqe_[9];

int _SmiN[9]; double _min[9], _min2[9], min_[9], min__[9], d_min[9], d_min_sum[9], q_min[9];      
int _Smax[9]; double _max[9], _max2[9], max_[9], max__[9], d_max[9], d_max_sum[9], q_max[9];
...
void fn_TTHAsub(int sw, int sw1, int sw3)
{
	...
	if(sw3<6)
	{
		...
		if (sw3==2)  stat_theta(s11_wert,a_M,a_THETS11);    
		if (sw3==3)  stat_theta(s12_wert,a_M,a_THETS12);  
		if (sw3==4)  stat_theta(s21_wert,a_N,a_THETS21);    
		if (sw3==5)  stat_theta(s22_wert,a_N,a_THETS22);  
	}
	
	if (sw3>=6)if(iLauf==1)                 { min__[sw3]=fn_erg; max__[sw3]=fn_erg;}  
...
void profil()
{
	...
	printf("Usage: Theta_rQ [sd][min][max][qq][qp][q11][q12][q21][q22][qr1][qr2][qQ]\n");
	printf("                [v][m][n][s] [[x]] [[g]]\n");
	...
	printf(" [qp]  ....................... Theta P/\n");
	printf(" [q11][q12] .................. Theta S11, S12/\n");
	printf(" [q21][q22] .................. Theta S21, S22:\n");
	...
	printf("                                |8| DvarO (DV)\n");
	printf(" [qr1][qr2] ...................Theta Regressionen 1,2/\n");
	printf("                                |1| Korrelation (kor)\n");
	printf("                                |2| Kovarianz (cov)\n");
	printf("                                |3| Determinatinskoeffizient (det)\n");
	printf("                                |4| Redundanz (red)\n");
	printf(" [qQ]  ....................... Theta Q:\n");
	printf("                                |1| Differenz (Diff)\n");
	printf("                                |2| Quotient (Quot)\n");
	printf("                                |3| Summe (Summ)\n");
	printf("                                |4| Produkt (Prod)\n");
	printf(" [v]  .......................  n zu Theta P (v)\n");
	printf(" [m]  .......................  n zu Theta S11,S12 (m)\n");
	printf(" [n]  .......................  n zu Theta S21,S22 (n)\n");
	printf(" [s]  .......................  n Subpopulationen (s)\n");
	printf(" [x]  .......................  Vergleichswert x\n");
	printf(" [g]  .......................  |1| Wertebereich ganzzahlig\n");
	for(iLauf=1; iLauf<=70; iLauf++) cprintf("\xC4"); printf("\n");
	printf("Theta_rQ by Dietmar Schrausser\n");
...
void kopf() 
{
	printf("\nTheta_rQ by Dietmar Schrausser\n");
	printf("compiled on %s @ %s\n", __DATE__, __TIME__);
	printf("computing Theta_rQ:\n\n"); 
}
