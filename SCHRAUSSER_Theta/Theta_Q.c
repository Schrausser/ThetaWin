...
{
	int   iLauf, jLauf, sLauf, mLauf, dLauf, zlr=0, zlr1=0, i;
	...
	char dbuffer [9],tbuffer [9], thetp[5], thets1[5], thets2[5], ttha[5], thetq[5];
	...
	outStreamR =  fopen( "Theta_Q_R.txt",   "w" );
	outStreamP =  fopen( "Theta_Q_P.txt",   "w" );
	outStreamS1 = fopen( "Theta_Q_S1.txt",  "w" );
	outStreamS2 = fopen( "Theta_Q_S2.txt",  "w" );
	outStreamQ =  fopen( "Theta_Q_Q.txt",   "w" );
	logStream  =  fopen( "Theta_Q_log.txt", "a" );
	...
	//log ausgabe
	                         fprintf (logStream,"SCHRAUSSER - Theta_Q");
	                         fprintf(logStream,"                         ");
	...
	for(dLauf=1;dLauf<=ds; dLauf++)                                                                                     //2 fach durchgang bei q=D=DV
	{
		...
		{
			for(sLauf=1;sLauf<=a_M+a_N; sLauf++)                                                                        //vektorerzeugung über mn
			{
			...
			for(mLauf=1;    mLauf<=a_M;    mLauf++)	s1_wert[mLauf]       = s_wert[mLauf];
			for(mLauf=a_M+1;mLauf<=a_M+a_N;mLauf++) s2_wert[mLauf-(a_M)] = s_wert[mLauf];
			...
			if(iLauf==1) {min__[4]= fn_erg; max__[4]= fn_erg;}
		...
		{
			q_min[1] = _min[1]/(a_S*(a_M+a_N));q_max[1] = _max[1]/(a_S*(a_M+a_N));qqe_[1]=sum[1]/(a_S*(a_M+a_N)); //sub v minimalwert mittelung//sub v maximalwert mittelung //kennwert mittelung 
	...
	//bildschirmausgabe
	                ...
	                printf("\n  \xB3");
	                printf("\n  \xC0\x10Q( %.2f, %.2f), %s(%s)= %.2f",             min__[4], max__[4], ttha, thetq,  qqe_[4]);
	if(argc >= 14 ) printf(", PR(%.2f)= %.2f",                                     a_X, pr_);
	
	//log ausgabe
	               fprintf(logStream,"\n:R\t%s\tv\t\t%.3f\t\t%.3f",                thetp,    q_min[1], q_max[1] );
	               fprintf(logStream,"\nP\t\ts(m+n)\t%.3f\t\t%.3f\t\t%.3f",        min__[1], max__[1],  qqe_[1] );
	               fprintf(logStream,"\n:P\t%s\tm\t\t%.3f\t\t%.3f",                thets1,   q_min[2], q_max[2] );
...
void fn_minwert(int sw)
{
	...
	{
		if(s1_wert[mLauf]<min_[sw]) min_[sw]= s1_wert[mLauf];//sub m minimalwert bestimmung
		if(s1_wert[mLauf]>max_[sw]) max_[sw]= s1_wert[mLauf];//sub m maximalwert bestimmung
	}

	if (sw==3)for(mLauf=1;mLauf<=a_N;mLauf++) 
	{
		if(s2_wert[mLauf]<min_[sw]) min_[sw]= s2_wert[mLauf];//sub n minimalwert bestimmung
		if(s2_wert[mLauf]>max_[sw]) max_[sw]= s2_wert[mLauf];//sub n maximalwert bestimmung
	}
}